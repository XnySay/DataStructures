...
public class Person extends Point {
    /**
     * 【功能】：整个程序的核心类，不仅有人的对象属性，还包含【人群移动算法】以及【感染传播的算法】方法。
     * 【思路】：
     * 1. 继承自Point，有坐标属性，新增City属性，
     *      新增人群移动意向（一些正太分布的均值和方差）人群根据这些参数进行随机生成移动的目的地。
     * 2. 新增接口State：采用数值方法，用于存放人群的不同的染病的状态。
     */
    private City city;
     ...
     * 以下为：【人群流动算法】
     *
     * 1. 计算人群的移动意愿，随机数，通过调整正态分布的均值改变流动概率，达到调整意愿目的
     * 2. 随机数产生新的目标地，计算当前和目标地的距离，判断移动方向
     * 3. 每次更新移动一点，最终达到目标地，并产生新的目标地，进行下一轮更新
     */
    /**
     * 流动意愿标准化
     * <p>
    ...
    /**
     * 以下为【感染传播算法】
     * 【功能】：对各种状态的人进行不同的处理，更新发布市民健康状态

     1. 设定一个安全距离，如果距离感染者距离<安全距离，根据设定的传播概率，选一个随机数乘以概率进行感染传播；
     2. 设定完成后，开始每轮更新；
     3. 每轮更新根据人群的状态进行不同的处理：
         1）隔离或者死亡：不需要处理
         2）确诊并且还没死亡：根据概率更新死亡，并且根据医院相应时间和床位进行住院隔离；
         3）正常人，计算最近的感染者距离进行随机感染。
